    ...
    try:
        ...
        if request.user.is_active and request.user.is_superuser:
            ...
            if 'auth/roles' in curr_url:
                ovc_id, ou_id = 0, 0
                au_id = item_id
            if '/ovc-care/' in curr_url:
                au_id, ou_id = 0, 0
                ovc_id = item_id
            if '/forms/' in curr_url:
                au_id, ou_id = 0, 0
                ovc_id = item_id
                is_dcs = True
            if ovc_id:
            ...
            if not is_allowed:
                ...
                if cdetails:
                    ...
                    if creator_id == user_id:
                        ...
                        # Implement workmate check
                        uid = check_workmate(user_id, item_id, perm_id)
                        if uid or creator_id == user_id:
                            is_allowed = True
                # Implement Geofencing to sub county level and below
            if is_allowed:
                cpgrp = get_groups('')
                ...
                if not u_grp:
                    is_allowed = False
            # Override for DCS on Forms in the meantime
            if is_dcs and not reg_ovc:
                is_allowed = True
    except Exception:
        return False
    else:
